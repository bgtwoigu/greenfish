...
type
  ...
  ICONDIR = packed record
    idReserved, idType, idCount: word;
  end;

  PICONDIRENTRY = ^ICONDIRENTRY;
  ICONDIRENTRY = packed record
    bWidth, bHeight, bColorCount, bReserved: byte;
    wPlanes_XHotSpot, wBitCount_YHotSpot: word; // [icon]_[cursor]
...
begin
  ...
  try
    ...
    if i shr 8 = PNG_Signature then
      Result := pngLoadFromStream(bm, s) else
      Result := dibRead(bm, s, -1, True);
...
begin
  ...
  try
    ...
    if PNGCompress then
    // write as PNG
      pngSaveToStream(bm, s, PNG_COMPRESSION_HIGH) else
  ...
  try
    ...
    try
      ...
      begin
        ...
        Page := Doc.NewPage;
        if not icoLoadPageFromStream(bm, Page.HotSpot, s,
