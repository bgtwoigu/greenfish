...
*)
// ICNS import/export
// TODO: handle 'clut' (Color lookup table) chunk
unit ICNS;
...
function icnsLoadFromStream(Doc: TIconDoc; s: TStream): boolean;
...
procedure icnsDecodeRawImage(bm: TBitmap32;
  const ri: TicnsRawImage; AsMask: boolean);
var
  ...
  case ri.Tag of
    ...
    begin
      ...
      try
        ...
        // try to decode image
        if not pngLoadFromStream(bm, s) and not jp2LoadFromStream(bm, s) then
          Exit; // there is nothing we can do
      finally
        s.Free;
      end;
    end;
  ...
  try
    ...
    for i := 0 to Length(RawImages) - 1 do
      ...
      begin
        ...
        try
          // Parse image data
          icnsDecodeRawImage(bm, ri^, False);
      ...
      begin
        ...
        try
          if ImageTag = it1024x1024_32 then
            pngSaveToStream(bm, st, PNG_COMPRESSION_HIGH)
          else
