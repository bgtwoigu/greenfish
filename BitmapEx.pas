...
type
  ...
  public
    ...
    // --- The following 4 functions must use RGBA blending,
  ...
  public
    ...
    // to position (x, y)
    procedure CopyRectOverwrite(x, y: integer; Src: TBitmap32; const rSrc: TRect);
    ...
    procedure DrawZoomedHatchedTo24(bm: TBitmap; xPos, yPos, Zoom: integer;
      Hatch: THatchDesc);
    // Draws only to pixels whose y is in the interval [yMin, yMax)
    // Shrink can be at most 100.
    procedure DrawShrunkHatchedTo24(bm: TBitmap; xPos, yPos, Shrink: integer;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        for x := 0 to Width - 1 do
        begin
          PColor32(p)^ := FlipColor(GetPixel(x, y));
          Inc(PColor32(p));
        end;
      end;
    finally
...
begin
  ...
  r.Right := Min(rSrc.Left + Width - x, bm.Width, rSrc.Right);
  if r.Right <= r.Left then Exit;
  r.Top := Max(0, rSrc.Top);
  r.Bottom := Min(rSrc.Top + Height - y, bm.Height, rSrc.Bottom);
  ...
  for j := r.Top to r.Bottom - 1 do
    PutPixel(x + i - r.Left, y + j - r.Top, bm.GetPixel(i, j));
end;
          ...
          begin
            ...
            for i := 0 to aaLevel - 1 do
              for j := 0 to aaLevel - 1 do
                Inc(Alpha, sl[(y2 - y) * aaLevel + i][((x2 - x) * aaLevel + j) * 3]);
            Alpha := Alpha shr (2 * log_aaLevel);
          end
          else
          if sl[y2 - y][(x2 - x) * 3] >= $80 then
            Alpha := 255
          else
            Alpha := 0;
...
begin
  ...
  begin
    ...
    try
      ...
      FWidth := NewWidth;
      FHeight := NewHeight;
      ReallocMem(FData, LineSize * Height);
      FillColor(cl32Transparent);
...
function TBitmap1.CreateBrushMask;
var
  i, mid, x, y, xLeft, yTop, x1, y1, x2, y2, Sqr_R: integer;
  q: double;
  xMin, xMax: TArrayOfInteger;
...
begin
  ...
  begin
    ...
    SetLength(xMin, BrushSize);
    SetLength(xMax, BrushSize);
    for y := 0 to BrushSize - 1 do
    begin
      q := Sqrt(Sqr_R - Sqr(2 * y - BrushSize + 1));
      xMax[y] := Floor((BrushSize - 1 + q) * 0.5);
      xMin[y] := Ceil((BrushSize - 1 - q) * 0.5);
    end;
  end;
    ...
    begin
      ...
      case BrushShape of
        bshRounded: for y := y1 to y2 do
            PaintHorizLine(xLeft + xMin[y - yTop], xLeft + xMax[y - yTop], y, cl32White);
        ...
        bshSpray: for y := y1 to y2 do
            for x := Max(0, xLeft + xMin[y - yTop]) to
              Min(Width - 1, xLeft + xMax[y - yTop]) do
...
// TODO: fix this workaround and use an own polygon scan converting algorithm
...
procedure TBitmap32.CopyRectOverwrite;
var
  i: integer;
  r: TRect;
...
begin
  ...
  r.Left := Max(0, rSrc.Left);
  r.Right := Min(rSrc.Left + Width - x, Src.Width, rSrc.Right);
  if r.Width <= 0 then Exit;
  r.Top := Max(0, rSrc.Top);
  r.Bottom := Min(rSrc.Top + Height - y, Src.Height, rSrc.Bottom);
  if r.Height <= 0 then Exit;

  x += r.Left - rSrc.Left;
  y += r.Top - rSrc.Top;

  if (x = 0) and (r.Left = 0) and (r.Right = Src.Width) and (Src.Width = Width) then
    Move(Src.PixelAddr(0, r.Top)^, PixelAddr(0, y)^, 4*Width*r.Height) else
  for i := r.Top to r.Bottom - 1 do
    Move(Src.PixelAddr(r.Left, i)^, PixelAddr(x, i-r.Top+y)^, 4*r.Width);
end;
...
begin
  ...
  Mask.Resize(Width, Height);
  Mask.FillColor(cl32Transparent);
  // range check
...
begin
  if Zoom = 1 then
  begin
    MakeTransparentHatch(bm, Rect(xPos, yPos, xPos + Width, yPos + Height), Hatch);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            Move3(@c[((y2 shr 2) xor (x2 shr 2)) and 1], pDest);
            Inc(PByte(pDest), 3);
          end;
        end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          c := Hatch.Color[(((yPos+y) shr 2) xor ((xPos+x) shr 2)) and 1];
          ...
          c := FlipColor(Put32to24(c, c32));
          MoveColor:
            Move3(@c, @sl[yPos + y][(xPos + x) * 3]);
...
begin
  ...
  try
    ...
    MakeTransparentHatch(bm, ClientRect, Hatch);
    DrawTo24(bm, 0, 0);
    c.Draw(x, y, bm);
  finally
    bm.Free;
