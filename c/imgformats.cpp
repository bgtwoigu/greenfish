...
typedef std::map<int64_t, void*> MyMap;

EXPORT void* mapCreate()
{
    return new MyMap();
}

EXPORT void mapDestroy(void* m)
{
    delete (MyMap*)m;
}

EXPORT void mapClear(void* m)
{
    ((MyMap*)m)->clear();
}

EXPORT void mapSet(void* m, int64_t key, void *value)
{
    (*(MyMap*)m)[key] = value;
}

EXPORT mybool mapContainsKey(void* m, int64_t key)
{
    MyMap &mm = (*(MyMap*)m);
    return (mm.find(key) != mm.end());
}

EXPORT void* mapGet(void* m, int64_t key)
{
    if (!mapContainsKey(m, key)) return NULL;
    return (*(MyMap*)m)[key];
}

EXPORT void mapForEach(void* m, MapCallback callback)
{
    MyMap &mm = (*(MyMap*)m);
    for (MyMap::iterator i = mm.begin(); i != mm.end(); i++)
        callback(i->second);
}
...
EXPORT mybool pngLoad(Color32 *bm, int32_t *w, int32_t *h, IOFunction readCallback, void *userData)
{
	// Clean up and return
...
EXPORT mybool pngSave(Color32 *bm, int32_t w, int32_t h, IOFunction writeCallback, void *userData, int32_t compressionLevel)
{
	// Clean up and return
