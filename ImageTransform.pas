...
begin
  ...
  try
    ...
    begin
      ...
      for x := 0 to w - 1 do
      begin
        bm.SetPixel(x, y, bmOrig.GetPixel(xSrc, ySrc));
...
var
  ...
  begin
    ...
    if Factor <> 0 then ca.Add(bmSrc.GetPixel(x1Int, y1Int), Factor);
    // top-right
    Factor := (x2Frac * ($100 - y1Frac)) shr 8;
    if Factor <> 0 then ca.Add(bmSrc.GetPixel(x2Int, y1Int), Factor);
    // bottom-left
    Factor := (($100 - x1Frac) * y2Frac) shr 8;
    if Factor <> 0 then ca.Add(bmSrc.GetPixel(x1Int, y2Int), Factor);
    // bottom-right
    Factor := (x2Frac * y2Frac) shr 8;
    if Factor <> 0 then ca.Add(bmSrc.GetPixel(x2Int, y2Int), Factor);

    // EDGES
    // left
    Factor := $100 - x1Frac;
    if Factor <> 0 then
      for i := y1Int + 1 to y2Int - 1 do ca.Add(bmSrc.GetPixel(x1Int, i), Factor);
    // top
    Factor := $100 - y1Frac;
    if Factor <> 0 then
      for i := x1Int + 1 to x2Int - 1 do ca.Add(bmSrc.GetPixel(i, y1Int), Factor);
    // right
    Factor := x2Frac;
    if Factor <> 0 then
      for i := y1Int + 1 to y2Int - 1 do ca.Add(bmSrc.GetPixel(x2Int, i), Factor);
    // bottom
    Factor := y2Frac;
    if Factor <> 0 then
      for i := x1Int + 1 to x2Int - 1 do ca.Add(bmSrc.GetPixel(i, y2Int), Factor);

    // SHEET
    for i := x1Int + 1 to x2Int - 1 do
    for j := y1Int + 1 to y2Int - 1 do
      ca.Add(bmSrc.GetPixel(i, j), $100);
          ...
          begin
            ...
            begin
              ...
              for i := -1 to 1 do for j := -1 to 1 do
                ca.Add(bmSrc.SafeGetPixel(xSrc_int+i, ySrc_int+j),
