...
function fltChannelFormula(bm: TBitmap32; Mask: TBitmap1;
  const fRed, fGreen, fBlue: string): boolean;
...
begin
  ...
  try
    ...
    case RetouchMode of
      ...
      rtmDodge: fltExposure(bm, BrushMask, Result, 1.06, 0);
      rtmBurn: fltExposure(bm, BrushMask, Result, 1 / 1.06, 0);
...
var
  ...
  p: PByteArray;
  sa: TffStrArray;
  ff: array[0..2] of TFloatFormula;
  q: array[0..2] of double;
  Stack: array[0..15] of double;
...
begin
  ...
  try
    ...
    sa[1] := 'g';
    sa[2] := 'b';
    if (ff[0].MakeFromString(fRed, sa) <> ffeNone) or
      (ff[1].MakeFromString(fGreen, sa) <> ffeNone) or
      (ff[2].MakeFromString(fBlue, sa) <> ffeNone) then
      Exit;
      ...
      for y := 0 to bm.Height - 1 do
        ...
        begin
          ...
          begin
            ...
            begin
              ff[i].Invoke(q, Stack);
              p[i] := Round(Max(0, Min(1, Stack[0])) * $ff);
            end;
          end;
  ...
  begin
    ...
    begin
      if i = 0 then
        ...
        Mapping[i] := Round(Exp(Ln(i / $ff) * Gamma) * $ff);
...
begin
  ...
  try
    // blurred shadow is black by default
    if Color <> clBlack then bmShade.Colorize(Color);
