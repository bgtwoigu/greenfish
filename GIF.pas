...
uses
  ...
  PixelFormats, ColQuant, Math, StreamEx, CIntf, dlgdebug;
...
procedure Map_FreeObject(p: Pointer);
begin
  TObject(p).Free;
end;
...
var
  ...
  MinCodeSize, BitCount, ResetCode, EndCode, FirstSequenceCode, FirstFreeCode: integer;
  InputByte: byte;
  InputBit: integer;
  lzwMap: Pointer;
  Input: string;
  ...
  begin
    mapForEach(lzwMap, @Map_FreeObject);
    mapClear(lzwMap);

    FirstFreeCode := FirstSequenceCode;
    BitCount := MinCodeSize + 1;

    // initialize LZW alphabet
    for i := 0 to (1 shl MinCodeSize) - 1 do
      mapSet(lzwMap, i, TlzwEntry.Create(-1, 1, i, i));
  end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        if mapContainsKey(lzwMap, Code) then
        begin
          Entry := mapGet(lzwMap, Code);

          // now we know the last data byte of the previously added seq
          if FirstFreeCode > FirstSequenceCode then
            TlzwEntry(mapGet(lzwMap, FirstFreeCode - 1)).LastData := Entry.FirstData;

          // augment the dictionary
          if FirstFreeCode < 4096 then
          begin
            mapSet(lzwMap, FirstFreeCode,
              TlzwEntry.Create(Code, Entry.Length + 1, Entry.FirstData, 0));
            inc(FirstFreeCode);
          ...
          begin
            ...
            Code := Entry.Parent;
            if Code >= 0 then Entry := mapGet(lzwMap, Code);
          end;
    ...
    begin
      ...
      case BlockMagic of
        $21: begin // extension block
          ...
          case BlockMagic2 of
            $01: begin // text
              // TODO process text extensions as well
        ...
        $2c: begin // image
          ...
          if Interlaced then Exit(False); // TODO handle interlaced files
...
var
  ...
  begin
    ...
    for i := 0 to (1 shl LogColorTableSize) - 1 do
      mapSet(lzwMap, (NullHash*HashPrime) xor i, Pointer(i));
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if not mapContainsKey(lzwMap, Hash) then
        begin
          AddToDictionary := True;
          Break;
        end;
        Code := Integer(mapGet(lzwMap, Hash));
        inc(Position);
      end;
      Print(Code);
      ...
      begin
        mapSet(lzwMap, Hash, Pointer(FirstFreeCode));
        inc(FirstFreeCode);
