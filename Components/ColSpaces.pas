...
begin
  cMax := Max(Max(r, g), b);
  cMin := Min(Min(r, g), b);
  Result[2] := ((cMax+cMin)*HSBMAX + RGBMAX) div (2*RGBMAX);
...
begin
  if Sat = 0 then
  begin
    Result[0] := DivByHSBMAX(Bri * RGBMAX);
    Result[1] := Result[0];
    Result[2] := Result[0];
  end else
  begin
    if Bri <= HSBMAX div 2 then
       n2 := DivByHSBMAX(Bri*(HSBMAX + Sat) + HSBMAX div 2) else
       n2 := Bri + Sat - DivByHSBMAX(Bri*Sat + HSBMAX div 2);

    n1 := 2*Bri - n2;

    Result[0] := DivByHSBMAX(HueToRGB(n1, n2, Hue + HSBMAX div 3) * RGBMAX +
      HSBMAX div 2);
    Result[1] := DivByHSBMAX(HueToRGB(n1, n2, Hue) * RGBMAX +
      HSBMAX div 2);
    Result[2] := DivByHSBMAX(HueToRGB(n1, n2, Hue - HSBMAX div 3) * RGBMAX +
      HSBMAX div 2);
  end;
end;
