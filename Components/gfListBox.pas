...
uses
  ...
  StdCtrls, Math, DoubleBufPB, dialogs;

// When refreshing the list box, call .ClientArea.Invalidate rather than .Invalidate
...
type
  ...
  TgflbItemPaint = procedure(Sender: TObject; ACanvas: TCanvas;
    Index: integer; ItemRect: TRect) of object;
...
function TgfListBox.GetSelected;
begin
  Result := False;
  if Assigned(OnItemGetSelected) then OnItemGetSelected(Self, Index, Result);
end;

procedure TgfListBox.SetSelected;
begin
  if not Assigned(OnItemSetSelected) then Exit;
  ...
  begin
    ...
    begin
      OnItemSetSelected(Self, ItemIndex, False);
...
function TgfListBox.GetItemIndex;
begin
  for Result := 0 to Count - 1 do if Selected[Result] then Exit;
  Result := -1;
end;

procedure TgfListBox.SetItemIndex;
var
  i: integer;
...
procedure TgfListBox.SetCanSelectNone;
begin
...
procedure TgfListBox.SetMultiSelect;
begin
...
procedure TgfListBox.SetLayout;
begin
...
procedure TgfListBox.SetItemWidth;
begin
...
procedure TgfListBox.SetItemHeight;
begin
...
procedure TgfListBox.SetColumns;
begin
...
procedure TgfListBox.caKeyDown;
var
...
procedure TgfListBox.caKillFocus;
begin
  ClientArea.Invalidate;
end;

procedure TgfListBox.caMouseDown;
var
  i, Index: integer;
  tc: Cardinal;
...
procedure TgfListBox.caMouseMove;
var
  i, Index: integer;
  r: TRect;
...
procedure TgfListBox.caMouseUp;
var
  Index: integer;
  r: TRect;
...
begin
  ...
  case ma of
    ...
    lbmaDragging: if Assigned(OnItemDragDrop) then
    begin
      OnItemDragDrop(Self, IfThen(Index < 0, Count, Index));
      ScrollTo(Index);
    end;
  end;

  ma := lbmaNone;
  ClientArea.Cursor := crDefault;
...
procedure TgfListBox.caMouseWheel;
begin
  ScrollBar.Position := ScrollBar.Position - Sign(WheelDelta) * 30;
  Handled := True;
end;

procedure TgfListBox.caPaint;
var
  i, iMin, iMax: integer;
  ...
  begin
    ...
    begin
      ...
      if (iMin >= 0) and (iMax >= 0) then for i := iMin to iMax do
        OnItemPaint(Self, ClientArea.Canvas, i, ItemRect(i));
    end;
...
procedure TgfListBox.caSetFocus;
begin
  ClientArea.Invalidate;
end;

procedure TgfListBox.ScrollBarChange;
begin
  ClientArea.Invalidate;
end;
...
function TgfListBox.LastSelected;
begin
  for Result := Count - 1 downto 0 do if Selected[Result] then Exit;
  Result := -1;
end;

function TgfListBox.SelectedCount;
var
  i: integer;
...
function TgfListBox.BorderSize;
begin
  Result := Point(1, 1);
end;

function TgfListBox.ListRect;
begin
  Result := ClientArea.ClientRect;
  InflateRect(Result, -BorderSize.X, -BorderSize.Y);
end;

function TgfListBox.LayoutOrigin;
begin
...
function TgfListBox.ItemVisible;
var
  i1, j1, i2, j2: integer;
  r: TRect;
...
function TgfListBox.FirstVisible;
var
  i: integer;
...
function TgfListBox.LastVisible;
var
  i: integer;
...
procedure TgfListBox.ScrollTo;
var
  i: integer;
  r: TRect;
  
begin
  if (Index >= 0) and (Index < Count) and not ItemVisible(Index, False) then
  begin 
    r := ItemRect(Index);
