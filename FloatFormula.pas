...
type
  ...
  public
    procedure Clear;
    function MakeFromString(const Code: string; VarNames: TffStrArray): TffError;
...
var
  i, p, p2, StackCount, fsp, fspMin, fspMax: integer;
  ...
  begin
    ...
    case Opcode.Instruction of
      ...
      ocAdd, ocSub, ocMul, ocDiv: ModifyFsp(2, 1);
      else ModifyFsp(1, 1);
    end;
  end;
  ...
  function Push(const Item: TStackItem): boolean;
  begin
    CurrOp := True;
...
begin
  ...
  // stack overflow?
  if (fspMax > 8) or (fsp > 1) then begin Result := ffeStackOverflow; Exit; end;
end;
